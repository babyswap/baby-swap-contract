...
contract BabyFarmV2 is SafeOwnable {
    ...
    modifier legalPid(uint _pid) {
        require(_pid > 0 && _pid < poolInfo.length, "illegal pid"); 
        _;
    }
    ...
    function disablePid(uint _pid) external onlyOwner {
        pidInBlacklist[_pid] = true;
    }

    function enablePid(uint _pid) external onlyOwner {
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyOwner legalPid(_pid) {
        require (_pid > 0 && _pid < poolInfo.length, 'pid begin at 1');
        if (_withUpdate) {
            massUpdatePools();
        }
